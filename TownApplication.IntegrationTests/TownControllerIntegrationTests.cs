namespace TownApplication.IntegrationTests
{
    public class TownControllerIntegrationTests
    {
        ...
        public void AddTown_ValidInput_ShouldAddTown()
        {
            ...
            var addedTown = _controller.GetTownByName(validTownName);
        ...
        public void AddTown_DuplicateTownName_DoesNotAddDuplicateTown()
        {
            ...
            var listTowns = _controller.ListTowns();
            ...
            Assert.Single(listTowns);
            Assert.Contains(listTowns, x => x.Name == townName && x.Population == 100);
        ...
        public void UpdateTown_ShouldUpdatePopulation()
        {
            ...
            var updateTown = _controller.GetTownByName(townName);
            ...
            Assert.NotNull(updateTown);
            Assert.Equal(updatePopulation, updateTown.Population);
            Assert.Equal(townName, updateTown.Name);
        ...
        public void DeleteTown_ShouldDeleteTown()
        {
            ...
            var deleteTown = _controller.GetTownByName(townName);
        ...
        public void ListTowns_ShouldReturnTowns()
        {
            ...
            var firstTown = "Pleven";
            var firstPopulation = 100;
            var secondTown = "Varna";
            var secondPopulation = 200;
            var thirdTown = "Sofia";
            var thirdPopulation = 300;

            // Act: Retrieve the list of towns from the database.

            _controller.AddTown(firstTown, firstPopulation);
            _controller.AddTown(secondTown, secondPopulation);
            _controller.AddTown(thirdTown, thirdPopulation);
            var listTowns = _controller.ListTowns();
            ...
            Assert.Equal(3, listTowns.Count);
            Assert.Contains(listTowns, x => x.Name == "Pleven" && x.Population == 100);
            Assert.Contains(listTowns, x => x.Name == "Varna" && x.Population == 200);
            Assert.Contains(listTowns, x => x.Name == "Sofia" && x.Population == 300);
